...
def check_product_details(link):
    try:
      #  scraper = cloudscraper.create_scraper()
        response = scraper.get(link, cookies=cookies_jar)
       
        response.raise_for_status()
        soup = BeautifulSoup(response.content, 'html.parser')

        # ابحث عن العنصر الذي يحتوي على النص "نفذ من المخزون" أو "OUT OF STOCK"
        out_of_stock_tag = soup.find('span', class_='bg-custom-orange-700')
        if out_of_stock_tag:
            stock_text = out_of_stock_tag.get_text(strip=True)
            if "نفذ من المخزون" in stock_text or "OUT OF STOCK" in stock_text:
                
                return False  # المنتج غير متوفر

        return True  # المنتج متوفر

    except requests.exceptions.RequestException as e:
        print(f"Error fetching product details: {e}", file=stderr)
        return False
    ...
    try:
        ...
        for product in products:
           ...
           # status_tag = product.find('span', class_='absolute right-0 top-0 px-1.5 font-semibold uppercase text-white shadow-xs bg-custom-orange-700')
            ...
            is_available = check_product_details(link)
            time.sleep(3)
